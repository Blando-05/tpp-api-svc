...
/**
 * Operations on /tppTransactionRequests/{ID}
 */
module.exports = {
  /**
   * summary: UpdateTransactionRequest
   * description: The HTTP request PUT /tppTransactionRequests/{ID} is used by the DFSP to inform the PISP about the result of a transaction request.
  ...
  put: async (_context: any, request: TraceableRequest, h: ResponseToolkit) => {
    const histTimerEnd = Metrics.getHistogram(
      'tpp_transaction_requests_put',
      'Put tpp transaction request by Id',
      ['success']
    ).startTimer()
    ...
    try {
      ...
      tppTransactionRequests.forwardTppTransactionRequests((<any> Enum.EndPoints.FspEndpointTemplates).TP_TRANSACTION_REQUEST_PUT, request.headers, Enum.Http.RestMethods.PUT, request.params, request.payload, span).catch((err: Error) => {
        // Do nothing with the error - forwardTppTransactionRequests takes care of async errors
