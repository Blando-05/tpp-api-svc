...
const TppTransactionRequests = require('../../../src/domain/tppTransactionRequests')
...
describe('tppTransactionRequests', () => {
  // URI
  ...
  describe('forwardTppTransactionRequests', () => {
    it('forwards a PUT request', async () => {
      ...
      const options = [
        ...
        'put',
        { ID: '12345' },
        { transactionRequestId: '12345', challenge: 'abc123', transactionId: 'txn123' },
        SpanMock
      ]

      // Act
      const result = await TppTransactionRequests.forwardTppTransactionRequests(...options)
    ...
    it('handles when the endpoint could not be found', async () => {
      ...
      sandbox.stub(TppTransactionRequests, 'forwardTppTransactionRequestsError').resolves({})
      sandbox.stub(Request, 'sendRequest').resolves({
        ok: true,
        status: 200,
        statusText: 'OK'
      ...
      // Act
      const action = async () => TppTransactionRequests.forwardTppTransactionRequests(...options)

      // Assert
      await expect(action()).rejects.toThrow(/No FSPIOP_CALLBACK_URL_TPP_REQ_SERVICE endpoint found for tppTransactionRequest/)
    })
    ...
    it('handles when the request fails and span is undefined', async () => {
      ...
      sandbox.stub(Endpoint, 'getEndpoint').resolves('http://localhost:3000')
      sandbox.stub(Request, 'sendRequest').throws(ErrorHandler.Factory.createFSPIOPError(ErrorHandler.Enums.FSPIOPErrorCodes.DESTINATION_COMMUNICATION_ERROR, 'Failed to send HTTP request to host', new Error(), '', [{ key: 'cause', value: {} }]))
      sandbox.stub(TppTransactionRequests, 'forwardTppTransactionRequestsError').resolves(true)
      const options = [
        Enum.EndPoints.FspEndpointTemplates.TP_TRANSACTION_REQUEST_PUT,
        TestHelper.defaultHeaders(resource, Config.PROTOCOL_VERSIONS),
        'put',
  ...
  describe('forwardTppTransactionRequestsError', () => {
    it('sends the error request', async () => {
      ...
      // Act
      const result = await TppTransactionRequests.forwardTppTransactionRequestsError(...options)
    ...
    it('handles when the endpoint could not be found', async () => {
      ...
      // Act
      const action = async () => TppTransactionRequests.forwardTppTransactionRequestsError(...options)
